...
#include "ARogPlayerState.generated.h"

// Event Handler for Credits
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FOnCreditsChanged, AARogPlayerState*, PlayerState, int32, NewCredits, int32, Delta);
// Event Handler for Personal Record Time
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FOnRecordTimeChanged, AARogPlayerState*, PlayerState, float, NewTime, float, OldRecord);
 ...
 */
UCLASS()
class ACTIONROGUELIKE_API AARogPlayerState : public APlayerState
{
	GENERATED_BODY()
...
public:
	...
	void SavePlayerState(UARogSaveGame* SaveObject);

	UFUNCTION(BlueprintNativeEvent)
	void LoadPlayerState(UARogSaveGame* SaveObject);
